...
struct client_data
{
    ...
    sockaddr_in address;   
    char* write_buf;           //接受其它会话消息
    char buf[BUFFER_SIZE];     //数据消息缓存内
};
    ...
    while(1)
    {
        ...
        for(int i = 0; i < user_counter+1; ++i)
        {
            ...
            if((fds[i].fd == listensock ) && (fds[i].revents & POLLIN))
            {
                ...
                {
                    /*  反馈在线人数以及在线用户ip  */
                    /*  最后改进时可以 考虑这里用dup占用stdout，而服务信息保存在一个日志文件中 */
                    char ip[1024] = "the online friend:\n";
                    ...
                    for(int i = 0; i < user_counter; i++)
                    {
                      ...
                      socklen_t peerlen = sizeof(struct sockaddr_in);
                        ...
                        char* user_ip = inet_ntoa(peer.sin_addr);
                        strcat(ip,user_ip);
                        strcat(ip,"\n");
                    }
                    
                    send(connfd,ip,strlen(ip)+1,0);
                    //尝试使用dup
                    //尝试失败！！！
            //        close(1);
            //        dup(connfd);
            ...
            //        for(int i = 0; i < user_counter; ++i)
            //        {
            //            int ret = getpeername(connfd,(struct sockaddr*)&peer,&peerlen);
            //            assert(!ret);
            ...
            {
                //类似于广播发送消息，通知每一个端口，改进的话可以使用udp群发；
                for(int broad = 1; broad <= user_counter; broad++)
                {
                    char quitmsg[100] = {0};
                    strcat(quitmsg,"the user is quit: ");
                    char* ret = strcat(quitmsg,inet_ntoa(users[fds[i].fd].address.sin_addr));
                    ret = strcat(ret,"\n");

                    send(fds[broad].fd,ret,strlen(quitmsg),0);
                }
                //客户端断开链接
